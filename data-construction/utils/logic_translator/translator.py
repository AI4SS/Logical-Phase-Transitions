...
class DataInspectionFailed(Exception):
    pass
...
class Translator:
    ...
    def translate_rules_and_facts(self, data) -> List:
        ...
        for i in problem_bar:
            ...
            if not background_story: continue # Skip if background generation failed
            ...
            if root_rule is None: continue # Skip if root rule translation failed
            ...
            for rule_id in range(1, len(problem.rules)):
                ...
                current_rule, updated_facts = self.__translate_normal_rule(current_rule, translated_facts, selected_name=selected_name, background_story=background_story, name_category=name_category)
                if current_rule is None or updated_facts is None:
                    translation_failed = True
                    logging.warning("Skipping problem due to normal rule translation failure.")
                    break  # Stop processing this problem if a normal rule fails
                translated_facts = updated_facts
            ...
            for idx in range(len(problem.facts)):
                if problem.facts[idx]['value'] != "Uncertain":
                    ...
                    # 添加检查：确保 fact_idx 存在于 translated_facts 中且不为 None
                    if fact_idx in translated_facts and translated_facts[fact_idx] is not None:
                        context_facts.append(translated_facts[fact_idx]['fact_nl'])
                        context_facts_fol.append(translated_facts[fact_idx]['fact'])
                    else:
                        logging.info(f"Fact with index {fact_idx} not found in translated_facts or is None. Skipping this fact.")
            # save result
    ...
    def __translate_root_rule(self, rule: Dict, name: str, background_story: str, name_category: str) -> Tuple:
        ...
        translate_logic_prompt = [
            {"role": "system", "content": (
                f"You will be provided a background story and a logic expression, your task is to replace the placeholders in the logic expression with appropriate words so that this logic expression represents a real world common sense rule and does not conflict with the common sense knowledge. "
                "The selected words should have no more than 5 words. If the chosen words can have some connection to the background story, that would be best, but it's okay if they don't. "
                "Note: Your answer should be in Valid JSON format. "
                "The keys should include 'universal_rule', 'specific_rule', and the placeholders (i.e. {rule_facts}) and the natural language of the rule"
            )}
        ]
        ...
        max_retries = 5
        expected_placeholders = set(rule_facts)
        for attempt in range(max_retries):
            try:
                result = self.__send_request(message=translate_logic_prompt)
                logging.debug(f"Model output JSON: {result}")
                # Ensure the model returned both universal_rule and specific_rule
                # Fallback: ensure both keys exist, derive specific_rule from universal_rule if missing
                missing = [key for key in ('universal_rule', 'specific_rule') if key not in result]
                if missing:
                    raise KeyError(f"Translator output missing keys: {missing}")
                actual_placeholders = {key for key in result.keys() if key not in ('universal_rule', 'specific_rule')}
                if actual_placeholders != expected_placeholders:
                    raise KeyError(f"Unexpected placeholder keys. Expected {expected_placeholders}, got {actual_placeholders}")
                break # Success
            except DataInspectionFailed:
                logging.warning("Data inspection failed in root rule, skipping this item.")
                return None, None, None, None
            except (KeyError, SyntaxError, ValueError) as e:
                logging.warning(f"Attempt {attempt + 1}/{max_retries} failed for root rule. Error: {e}. Retrying...")
                if attempt + 1 == max_retries:
                    logging.error(f"Failed to translate root rule after {max_retries} attempts. Skipping.")
                    return None, None, None, None
                time.sleep(1)
    ...
    def __translate_normal_rule(self, rule: Dict, translated_facts: Dict, selected_name: str, background_story: str, name_category: str) -> Tuple:
        ...
        max_retries = 5
        attempt = 0
        expected_placeholders = {item for item in rule_fact_placeholder if item not in ['universal_rule', 'specific_rule']}
        while has_repetition and attempt < max_retries:
            has_repetition = False
            try:
                result = self.__send_request(message=translate_logic_prompt)
                logging.debug(f"Model output JSON: {result}")
                
                # Basic validation for presence of required keys
                if "universal_rule" not in result or "specific_rule" not in result:
                    raise KeyError("Model output missing 'universal_rule' or 'specific_rule'")
                actual_placeholders = {key for key in result.keys() if key not in ["universal_rule", "specific_rule"]}
                if actual_placeholders != expected_placeholders:
                    raise KeyError(f"Unexpected placeholder keys. Expected {expected_placeholders}, got {actual_placeholders}")
                ...
                for key in result.keys():
                    ...
                    if result[key] in forbidden_list or re.findall(r'^F\d+$', result[key]) != []:
                        has_repetition = True
                        logging.warning(f"Repetitive or invalid predicate found: {result[key]}. Retrying...")
                        break
                ...
                logging.warning(f"Attempt {attempt + 1}/{max_retries} failed for normal rule. Error: {e}. Retrying...")
                has_repetition = True # Force retry

            attempt += 1
            if attempt >= max_retries:
                logging.error(f"Failed to translate normal rule after {max_retries} attempts. Skipping.")
                return None, None
            time.sleep(1)
    ...
    def __retrieve_fact_from_rules(self, logic_expression: str, rule_nl: str, predicate: str, value: str) -> Tuple:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 'query_natural_language' not in result:
                    raise KeyError("Model output missing 'query_natural_language'")
                return result['query_natural_language'], value
            except DataInspectionFailed:
                logging.warning("Data inspection failed in root rule, skipping this item.")
                return "", value
            except (KeyError, SyntaxError, ValueError) as e:
                logging.warning(f"Attempt {attempt + 1}/{max_retries} failed for fact retrieval. Error: {e}. Retrying...")
                if attempt + 1 == max_retries:
                    logging.error(f"Failed to retrieve fact after {max_retries} attempts. Returning empty string.")
                    return "", value
                time.sleep(1)
    ...
    def __generate_background(self, name: str) -> Tuple:
        ...
        prompt = [
            {"role": "system", "content": "You will be given a keyword and a name (can be a person's name or an animals' name). Your tasks is to generate a philosophical background story with no more than 150 words according to the keyword about this entity.Your answer should be in JSON format with keys: category, story."}
        ]
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 'category' not in result or 'story' not in result:
                    raise KeyError("Model output missing 'category' or 'story'")
                return result['category'], result['story'], keyword
            except DataInspectionFailed:
                logging.warning("Data inspection failed in root rule, skipping this item.")
                return "", "", keyword
            except (KeyError, SyntaxError, ValueError) as e:
                logging.warning(f"Attempt {attempt + 1}/{max_retries} failed for background generation. Error: {e}. Retrying...")
                if attempt + 1 == max_retries:
                    logging.error(f"Failed to generate background after {max_retries} attempts. Returning empty strings.")
                    return "", "", keyword
                time.sleep(1)
    ...
    def __send_request(self, message: List) -> str:
        while True:
            ...
            try:
                ...
                logging.error(f"JSON decoding failed: {e}. Response: {answer_str}")
                self.err_cnt += 1
                time.sleep(2)
            except Exception as e:
                msg = str(e)
                if 'data_inspection_failed' in msg or 'Input data may contain inappropriate content.' in msg:
                    logging.error(f"Data inspection failed, skipping this data. {e}")
                    raise DataInspectionFailed()
                self.err_cnt += 1
                logging.error(f"API error occurred: {e}. Error count: {self.err_cnt}")
                time.sleep(3)
