# ProntoQA with Chain-of-Thought (Ollama Version)
import argparse
import json
import time
import re
...
def get_parser():
    ...
    parser.add_argument('--dataset', type=str, default='data/dev.json', help='dataset to use')
...
def get_cot_prompt(premises, conclusion):
    """Create Chain-of-Thought prompt for ProntoQA"""
    prompt = f"""Suppose you are one of the greatest AI scientists, logicians and mathematicians. Let's think step by step.

Read and analyze the "Premises" first, then using logical reasoning to judge whether the "Hypothesis" is True or False.
...
Provide consise reasoning with no more than 3 sentences and finally give the judgement.

Example:
Premises: "All scientists are curious. Marie is a scientist. All curious people ask questions."
Hypothesis: "Marie asks questions."
Output: {{"reasoning": "Marie is a scientist, and all scientists are curious. Since curious people ask questions, Marie asks questions.", "answer": "True"}}

Example:
Premises: "No fish can fly. All sharks are fish. Dolphins are not fish."
Hypothesis: "Sharks can fly."
Output: {{"reasoning": "All sharks are fish and no fish can fly. Therefore, sharks cannot fly, making the statement false.", "answer": "False"}}

Example:
Premises: "All mammals are warm-blooded. All dogs are mammals. Fido is a dog."
Hypothesis: "Fido is warm-blooded."
Output: {{"reasoning": "Fido is a dog and all dogs are mammals. Since all mammals are warm-blooded, Fido is warm-blooded.", "answer": "True"}}
...
def parse_response(response):
    ...
    try:
        ...
        # Look for answer - ProntoQA only has True or False
        answer = "False"  # Default to False for ProntoQA
...
def main():
    ...
    # Preprocess data
    for item in data:
        # ProntoQA specific format
        item['premises'] = item.get('context', '')
        # Extract the statement from the question
        question = item.get('question', '')
        if 'Is the following statement true or false?' in question:
            # Extract the actual statement after the question
            parts = question.split('Is the following statement true or false?')
            if len(parts) > 1:
        ...
        # Map the answer - ProntoQA only has True or False
        ...
        if answer == 'A' or answer == 'True':
            ...
            item['label'] = 'False'  # Default to False for ProntoQA
    ...
    if args.output_file:
        ...
        logfilename = f'results/prontoqa/results-prontoqa-cot-ollama--{model_name}--t{args.temperature}--{dataset_name}--k_{args.majoritycnt}--{time.strftime("%Y-%m-%d-%H-%M-%S", t)}.jsonl'
    ...
    for example in tqdm(data, desc="Evaluating", unit="example"):
        ...
        # Majority vote for judgement - ProntoQA only has True or False
        ...
        for i in range(args.majoritycnt):
            try_cnt = 0
            reasoning = ""
            judgement = "False"  # Default to False for ProntoQA
            ...
            while try_cnt < TRY_CNT:
                try:
                    ...
                    # Validate judgement - ensure only True or False
