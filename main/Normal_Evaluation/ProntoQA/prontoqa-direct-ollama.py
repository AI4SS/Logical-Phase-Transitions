# ProntoQA with Direct (Ollama Version) - No reasoning
import argparse
import json
import time
import re
...
def get_direct_prompt(premises, conclusion):
    ...
    prompt = f"""Based only on the given premises, determine if the hypothesis is True or False.

Premises: {premises}

Hypothesis: {conclusion}

Based strictly on the premises provided, is the hypothesis True or False? Answer with only "True" or "False".
...
def parse_response(response):
    ...
    # Check if response contains both true and false, use the last occurrence
    true_pos = response.rfind("true")
    false_pos = response.rfind("false")

    if true_pos > false_pos and true_pos != -1:
        return "True"
    elif false_pos > true_pos and false_pos != -1:
        return "False"

    # Check first word if nothing else works
    if words:
...
def main():
    ...
    if args.output_file:
        ...
        logfilename = f'results/prontoqa/results-prontoqa-direct-ollama--{model_name}--t{args.temperature}--{dataset_name}--k_{args.majoritycnt}--{time.strftime("%Y-%m-%d-%H-%M-%S", t)}.jsonl'
