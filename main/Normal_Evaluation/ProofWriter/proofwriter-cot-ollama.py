# ProofWriter with Chain-of-Thought (Ollama Version)
import argparse
import json
import time
import re
...
def get_parser():
    ...
    parser.add_argument('--dataset', type=str, default='data/proofwriter/proofwriter-dev.json', help='dataset to use')
...
def main():
    ...
    # Preprocess data
    for item in data:
        # ProofWriter specific format
        item['premises'] = item.get('context', '')
        # Extract the statement from the question
        question = item.get('question', '')
        if 'Based on the above information, is the following statement true, false, or unknown?' in question:
            # Extract the actual statement after the question
            parts = question.split('Based on the above information, is the following statement true, false, or unknown?')
            if len(parts) > 1:
    ...
    if args.output_file:
        ...
        logfilename = f'results/proofwriter/results-proofwriter-cot-ollama--{model_name}--t{args.temperature}--{dataset_name}--k_{args.majoritycnt}--{time.strftime("%Y-%m-%d-%H-%M-%S", t)}.jsonl'
